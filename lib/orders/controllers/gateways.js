  ...
  if (process.env.BTCSECRET != secret) {
    ...
    if (req.query.confirmations > 6) {
      ...
      Order.findById(req.params.order, function(err, order) {
        ...
        var remaining = payment.provider.data.payed != undefined ? payment.amount - payment.provider.data.payed : payment.amount;
        ...
        if (remaining == satoshi) {
          ...
          Account.findById(req.params.customer, function(err, customer) {
            if (err)
              throw new Error(err);
            if (remaining > satoshi) {
              payment.provider.data.payed = payment.provider.data.payed || 0;
              payment.provider.data.payed += satoshi;
              var payment_obj = {
                customer : customer.toObject(),
                order : {
                  id : order._id,
                  grandTotal : (payment.amount - satoshi) / 100000000,
              ...
              var refund_obj = {
                ...
                refund : btc - (remaining / 100000000)
              };
