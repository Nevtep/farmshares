  ...
  if (process.env.BTCSECRET != secret) {
    ...
    if (req.query.confirmations > 6) {
      ...
      Order.findById(req.params.order, function(err, order) {
        ...
        var remaining = _.first(order.payments).provider.data.payed != undefined ? order.payments[0].amount - order.payments[0].provider.data.payed : order.payments[0].amount;
