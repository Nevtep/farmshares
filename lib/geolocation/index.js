...
exports.middleware = function (req, res, next) {
  ...
  req.geo = geoip.lookup(req.ip) || {country:"AR",ll:[0,0]};
  next();
}
...
exports.geoFilterFarms = function (geo, options, cb) {
  ...
  if( !geo || !geo.ll || !cb) {
    ...
    var conditions = {"location.country.shortname" : geo.country}
    
    if(!_.isEqual(geo.ll,[0,0]))
      _.extend(conditions, {
        "location.geometry" : {
...
exports.geoFilterSKUs = function (geo, options, cb) {
  ...
  if( !geo || !geo.ll || !cb) {
    ...
    // This is pretty easier than what it sounds like
    winston.info("Geocords:", geo.ll);
    var geoConditions = {"location.country.shortname" : geo.country}
    
    if(!_.isEqual(geo.ll,[0,0]))
      _.extend(geoConditions, {
        "location.geometry" : {
          $nearSphere: [geo.ll[1], geo.ll[0]], //lat/lon to lon/lat
          $maxDistance: 10
        }
