...
var uploadFile = function (uploadpath, file, S3, callback) {
    ...
    if (process.env.NODE_ENV !== "development") {
        ...
        winston.info("Uploaded size:", file.size)
        ...
        SKU.findById(skuid, function (err, sku) {
            if (err) {
              ...
              if(!sku){
                ...
                if (!share) {
                    winston.error("Couldn't find share in sku shares.", sku.toObject().shares, shareid);
                    res.json({
                        files: [
                            ]
                    });
                } else {
                    winston.info("Found share, pushing image", share.toObject());
                  ...
                  sku.markModified("shares");
                    sku.save(function (err) {
                        if (err) {
                            ...
                            winston.info("New photo \"" + path + "\" succesfully added to share.", sku.toObject().shares, share.toObject());
                            res.json({
                                files: [
                                {
                                    name: file.name,
